...
CREATE TABLE BandGenres(band_id int, genre_id int, FOREIGN KEY(band_id) REFERENCES Band(band_id),
FOREIGN KEY(genre_id) REFERENCES Genre(genre_id), PRIMARY KEY(band_id, genre_id));

CREATE TABLE ShowsBands(show_id int, band_id int, FOREIGN KEY(show_id) REFERENCES Shows(show_id),
FOREIGN KEY(band_id) REFERENCES Band(band_id), PRIMARY KEY(show_id, band_id));

CREATE TABLE UserID(user_id int AUTO_INCREMENT, user_email varchar(50) NOT NULL, user_password varchar(50) NOT NULL,
PRIMARY KEY(user_id));
